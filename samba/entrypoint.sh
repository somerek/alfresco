...
jq -c '.personal | .[]' /ftp_acc/users.json | while read i; do
    ...
    password=`echo $i | jq '.password'| sed 's/^"\(.*\)"$/\1/'`
    echo "  - user: $username" >> $config_file
    echo "    group: $username" >> $config_file
    echo "    password: $password" >> $config_file
    echo "    uid: $n" >> $config_file
    echo "    gid: $n" >> $config_file
    ((n=n+1))
done
...
jq -c '.personal | .[]' /ftp_acc/users.json | while read i; do
    ...
    echo "    validusers: $username" >> $config_file
    echo "    writelist: $username" >> $config_file
    echo "    veto: no" >> $config_file
done
