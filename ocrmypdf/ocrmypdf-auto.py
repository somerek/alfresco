...
class OcrTask(object):
    ...
    def process(self, skip_delay=False):
        ...
        if rc == 0:
            ...
            username = self.input_path.split('/')[1]
            with open('/ftp_acc/users.json', 'r', encoding='utf-8') as f:  # открыли файл с данными
            ...
            for user in users:
                if user['username'] == username:
                    ...
                    phoneIP = user['phoneIP'] if 'phoneIP' in user else ''
                    phoneNum = user['phoneNum'] if 'phoneNum' in user else ''
                    # upload to ftp alfresco
                    os.system(
                        '/app/ftp_upload.sh \"' + self.input_path + '\" \"' + self.output_path + '\" ' + username + ' password ' + bitrixId + ' ' + phoneIP + ' ' + phoneNum)
                    break
        else:
            self.logger.warning('OCRmyPDF failed with rc %d stdout [%s] and stderr [%s]', rc, stdout, stderr)
